...
> 面向 AI 岗位求职作品集。本文从「AI / 大模型」与「工程开发」两个视角完整介绍该系统，可作为面试讲解底稿与简历技术亮点来源。
...
### 3.5 人工复核闭环 HIL（Human-in-the-Loop，面试高光）
...
- **为什么用检索投票而不是微调/训练分类器？** 少样本、冷启动友好、可解释、可在线学习。
- **余弦相似度的局限？** 高维稀疏向量区分度下降；稠密 embedding 更优——已留可插拔接口。
- **维度灾难**：TF-IDF 维度=词表，稀疏高维；真实 embedding 固定维度（如 1536）稠密，需归一化。
- **HIL 阈值怎么定？** 用评测集置信度分布 + 误分流率反推，0.55 是权衡值。
- **主动学习采样策略？** uncertainty sampling（挑低置信/高熵样本）；也可 margin sampling。
- **冷启动怎么办？** 种子语料 + 默认规则兜底 + 人工复核兜底。
